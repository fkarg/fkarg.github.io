...
Timestamp of last backup: 2021-03-16 10:47

Did significant changes happen since the last backup: absolutely.

### Versioning (v1.1)
So I started to add versioning numbers to some headers. Version `1.0` is everything that was present when we restructured. I have marked parts where I have read through and made _some_ changes with `v1.1`.

Only the lowest-level paragraph header is versioned, in cases where this is not the case, no changes have been made since.

Paragraphs that are deemed '**final**' should be versioned as `final` and not changed thereafter.
...
---
# Intro: About Our Group / How We formed
Everything takes place within Germany.
...
Talking about Hammertime takes sometimes more but usually less time. It takes longer when we have more discussions about related topics, and not Hammertime itself (related: [Allow Topic Digressions](#allow-topic-digressions)).
...
Our cadence was quite high initially, but slowed down with more people. This is a tradeoff we're willing to make, if it means that everyone (us included!) continues to participate and make progress.

#### vorträge (v1.0)
#### opinion speed dating (v1.0)
